...
#define READ_DATA 142

#define SENSOR_DIRT 15
#define CHARGING_STATE 21

#define CHARGING_CAPACITY 26
#define MOTOR_CURRENT 57
...
// Write to the port
void cmdRoombaZero(int command)
{
    Serial1.write(command);
    //delay(100);
}
 ...
 **---------------------------------------------------------------------------*/
void roombaCmdAllStop(int *roombaState, din7Connector roombaDin, int cmdOpCode)
{
...
void roombaCmdFromClean(int *roombaState, din7Connector roombaDin, int cmdOpCode)
{
    ...
    if (cmdOpCode == CLEAN)
        Serial.println("Already cleaning...");
    if (cmdOpCode == SEEK_DOCK)
    {
...
void roombaCmdFromSeek(int *roombaState, din7Connector roombaDin, int cmdOpCode)
{
    ...
    if (cmdOpCode == SEEK_DOCK)
        Serial.println("Already seeking dock...");
    if (cmdOpCode == CLEAN)
    {
